...
def test_tool_interface_agent_input_schema(tool_interface_agent):
    ...
    assert tool_interface_agent.input_schema.model_config['title'] == "MockTool"
    assert "A mock tool for testing" in tool_interface_agent.input_schema.model_config['description']
...
def test_tool_interface_agent_get_response_without_raw_output(mock_format_tool_message, mock_get_response, tool_interface_agent):
    ...
    mock_format_tool_message.return_value = {"id": "test_id", "type": "function", "function": {"name": "MockTool", "arguments": "{}"}}
...
def test_tool_interface_agent_get_response_with_raw_output(mock_format_tool_message, mock_get_response, mock_tool, mock_instructor):
    ...
    mock_tool_input = agent.input_schema(tool_input_MockTool="Test query")
    mock_tool_output = MockTool.OutputSchema(result="Mocked result")

    mock_get_response.return_value = mock_tool_input
    agent.tool_instance.run = Mock(return_value=mock_tool_output)
    mock_format_tool_message.return_value = {"id": "test_id", "type": "function", "function": {"name": "MockTool", "arguments": "{}"}}

    response = agent.get_response()

    agent.tool_instance.run.assert_called_once_with(mock_tool_input)
    assert mock_get_response.call_count == 1
    assert response == mock_tool_output
