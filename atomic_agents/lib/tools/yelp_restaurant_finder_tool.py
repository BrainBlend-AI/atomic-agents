...
class YelpSearchToolInputSchema(BaseIOSchema):
    ...
    price: Optional[List[PriceRange]] = Field(
        None,
        description=(
            "Price range to filter by (e.g., '1', '2', '3', '4'). "
            "Can be multiple. 1 is cheap. 2 and 3 are mid-range. 4 is the most high-end."
        ),
    )
    open_now: Optional[bool] = Field(False, description="Filter for businesses that are open now.")
    sort_by: Optional[str] = Field(
        "best_match",
        description="Sort by criteria (e.g., 'best_match', 'rating', 'review_count', 'distance').",
    )
