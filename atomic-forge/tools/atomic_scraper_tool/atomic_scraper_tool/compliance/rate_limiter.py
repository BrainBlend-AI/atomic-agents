    ...
    def get_semaphore(self, domain: str) -> threading.Semaphore:
        ...
        with self._lock:
            if domain not in self._semaphores:
                self._semaphores[domain] = threading.Semaphore(self.config.max_concurrent_requests)
    ...
    def apply_backoff(self, domain: str, is_failure: bool = False):
        ...
        if is_failure:
            ...
            if stats.consecutive_failures > 0:
                ...
                stats.current_delay = max(self.config.default_delay, stats.current_delay * 0.8)
...
class RespectfulCrawler:
    ...
    def prepare_request(self, url: str, user_agent: str = None) -> Tuple[bool, float]:
        ...
        if not self.rate_limiter.acquire_request_slot(url):
            raise ScrapingError(
                "Request not allowed: Maximum concurrent requests reached for domain",
                "rate_limit_error",
                url,
            )
