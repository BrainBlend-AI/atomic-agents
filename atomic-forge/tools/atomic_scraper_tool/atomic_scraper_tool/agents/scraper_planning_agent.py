...
class AtomicScrapingContextProvider(BaseDynamicContextProvider):
...
class AtomicScraperPlanningAgent(
    AtomicAgent[AtomicScraperAgentInputSchema, AtomicScraperAgentOutputSchema]
):
    """
    ...
    def __init__(self, config: AgentConfig):
        ...
        # Update config with system prompt generator (schemas are now type parameters)
    ...
    def _create_basic_schema_recipe(
        self, parsed_request: Dict[str, Any], input_data: AtomicScraperAgentInputSchema
    ) -> "SchemaRecipe":
    ...
    def _generate_reasoning(
        ...
        reasoning_parts.append(
            f"- Target data types: {', '.join(parsed_request['target_data']) if parsed_request['target_data'] else 'general content'}"
        )
        reasoning_parts.append(
            f"- Key terms extracted: {', '.join(parsed_request['keywords'][:5]) if parsed_request['keywords'] else 'none specific'}"
        )
