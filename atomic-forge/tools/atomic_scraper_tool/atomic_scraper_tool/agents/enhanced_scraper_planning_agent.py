...
class EnhancedScraperPlanningAgent(AtomicScraperPlanningAgent):
    ...
    def _generate_scraping_strategy(
        self, website_analysis: AdaptiveAnalysisResult, parsed_request: Dict[str, Any], input_data: ScraperPlanningInputSchema
    ) -> ScrapingStrategy:
        """
        ...
        if website_analysis.has_enhanced_analysis:
            ...
            base_strategy = self._enhance_strategy_with_navigation_analysis(base_strategy, enhanced_analysis, parsed_request)

        return base_strategy

    def _enhance_strategy_with_navigation_analysis(
        self, base_strategy: ScrapingStrategy, enhanced_analysis, parsed_request: Dict[str, Any]
    ) -> ScrapingStrategy:
        """
    ...
    def _update_selectors_from_navigation(self, strategy: ScrapingStrategy, nav_hierarchy):
        ...
        if hasattr(strategy, "content_selectors") and nav_hierarchy.items:
            ...
            main_content_indicators = ["main", "#main", ".main-content", ".content", "article", ".article", ".post", ".entry"]
    ...
    def _generate_navigation_insights(self, analysis_result: AdaptiveAnalysisResult) -> str:
        ...
        insights.append(
            f"**Website Complexity**: {analysis_result.navigation_complexity.title()} (score: {analysis_result.complexity_score:.2f})"
        )
    ...
    def _generate_enhanced_recommendations(
        self, website_analysis: AdaptiveAnalysisResult, strategy: ScrapingStrategy
    ) -> List[str]:
        ...
        if website_analysis.has_enhanced_analysis:
            ...
            if enhanced.mega_menus:
                recommendations.append(
                    "Consider scraping category hierarchies from mega menus for comprehensive data coverage"
                )
