...
class AtomicScraperApp:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "agent": {"model": "gpt-3.5-turbo", "temperature": 0.7, "max_tokens": 2000},
            "interface": {
                "show_reasoning": True,
...
- "Extract article titles and dates from this news website"
    ...
    def _show_main_menu(self):
        ...
        panel = Panel(menu_table, title="Main Menu", border_style="green", padding=(0, 1))
    ...
    def _display_scraping_results(self, scraping_result, original_request: str):
        ...
        # Summary panel
        summary_panel = Panel(scraping_result.summary, title="Summary", border_style="green")
    ...
    def _initialize_planning_agent_with_client(self, client):
        ...
        try:
            ...
            agent_config = AgentConfig(client=client, model=self.config.get("agent", {}).get("model", "gpt-3.5-turbo"))
...
def main():
    ...
    try:
        ...
        app = AtomicScraperApp(config_path=args.config, client=client, demo_mode=args.demo)
        if args.debug:
            app.debug_mode = True
        app.run()
    except KeyboardInterrupt:
...
def create_orchestrated_app(
    config: Optional[Dict[str, Any]] = None, client: Optional[Any] = None, demo_mode: bool = False
) -> AtomicScraperApp:
    """
