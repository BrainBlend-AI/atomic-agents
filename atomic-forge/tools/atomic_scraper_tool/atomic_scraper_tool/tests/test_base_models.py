...
class TestScrapingStrategy:
    ...
    def test_invalid_scrape_type(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScrapingStrategy(scrape_type="invalid_type", target_selectors=[".item"])
    ...
    def test_empty_target_selectors(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScrapingStrategy(scrape_type="list", target_selectors=[])
    ...
    def test_invalid_css_selector(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScrapingStrategy(scrape_type="list", target_selectors=["<invalid>selector"])
    ...
    def test_field_constraints(self):
        ...
        with pytest.raises(ValidationError):
            ScrapingStrategy(scrape_type="list", target_selectors=[".item"], max_pages=0)

        # Test request_delay minimum value
        with pytest.raises(ValidationError):
            ScrapingStrategy(scrape_type="list", target_selectors=[".item"], request_delay=0.05)
...
class TestScrapedItem:
    ...
    def test_invalid_source_url(self):
        ...
        # Empty URL
        with pytest.raises(ValidationError) as exc_info:
            ScrapedItem(source_url="", data={"title": "Test"}, quality_score=75.0)

        assert "source_url cannot be empty" in str(exc_info.value)

        # Invalid URL format
        with pytest.raises(ValidationError) as exc_info:
            ScrapedItem(source_url="not-a-url", data={"title": "Test"}, quality_score=75.0)

        assert "Invalid URL format" in str(exc_info.value)

        # Invalid scheme
        with pytest.raises(ValidationError) as exc_info:
            ScrapedItem(source_url="ftp://example.com", data={"title": "Test"}, quality_score=75.0)
    ...
    def test_empty_data(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScrapedItem(source_url="https://example.com", data={}, quality_score=75.0)
    ...
    def test_invalid_data_types(self):
        ...
        # Non-string keys
        with pytest.raises(ValidationError) as exc_info:
            ScrapedItem(source_url="https://example.com", data={123: "value"}, quality_score=75.0)
    ...
    def test_valid_data_types(self):
        ...
        item = ScrapedItem(source_url="https://example.com", data=valid_data, quality_score=75.0)
...
class TestScrapingResult:
    ...
    def create_sample_strategy(self) -> ScrapingStrategy:
        ...
        return ScrapingStrategy(scrape_type="list", target_selectors=[".item"])
