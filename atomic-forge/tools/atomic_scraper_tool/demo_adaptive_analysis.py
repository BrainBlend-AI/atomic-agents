...
def demo_conditional_logic():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if result.analysis_type == "enhanced":
            print(f"      ✅ Enhanced analysis triggered because:")
            if result.complexity_score >= config.min_page_complexity_score:
                print(f"         • Complexity score ({result.complexity_score:.3f}) >= threshold ({config.min_page_complexity_score})")
            ...
            print(f"      ⚡ Standard analysis used because:")
            print(f"         • Complexity score ({result.complexity_score:.3f}) < threshold ({config.min_page_complexity_score})")
            print(f"         • No complex navigation features detected")
        ...
        if result.has_enhanced_analysis:
            ...
            if enhanced.mobile_navigation:
                print(f"      • Mobile navigation (hamburger menu)")
