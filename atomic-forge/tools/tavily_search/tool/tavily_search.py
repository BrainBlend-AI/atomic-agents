...
class TavilySearchTool(BaseTool):
    ...
    async def _fetch_search_results(
        self, session: aiohttp.ClientSession, query: str, max_results: Optional[int]
    ) -> List[dict]:
        headers = {
            "accept": "/",
    ...
    async def run_async(
        ...
        async with aiohttp.ClientSession() as session:
            ...
            tasks = [
                self._fetch_search_results(session, query, max_results if max_results is not None else self.max_results)
